...
Build system patterns following convention plugins approach, updated for AGP 9.0.
...
6. [AGP 9.0 Migration Notes](#agp-90-migration-notes)
...
│   ├── libs.versions.toml
│   └── wrapper/
│       └── gradle-wrapper.properties  # Must use Gradle 9.1.0+
├── build.gradle.kts
└── settings.gradle.kts
```
...
androidx-test-ext = { group = "androidx.test.ext", name = "junit-ktx", version = "1.2.1" }
...
class AndroidLibraryConventionPlugin : Plugin<Project> {
    override fun apply(target: Project) {
        with(target) {
            // AGP 9.0: Kotlin support is built-in, no need to apply kotlin-android
...
class AndroidLibraryComposeConventionPlugin : Plugin<Project> {
    override fun apply(target: Project) {
        with(target) {
            pluginManager.apply("com.android.library")
            // Compose compiler is now a Kotlin compiler plugin (Kotlin 2.0+)
...
internal fun Project.configureAndroidCompose(
    ...
    commonExtension.apply {
        ...
        // No composeOptions block needed — the kotlin-compose plugin handles
        // compiler version alignment automatically.
...
Key changes when migrating to Android Gradle Plugin 9.0:

1. **Kotlin is built-in**: Remove `id("org.jetbrains.kotlin.android")` from plugin blocks — AGP 9.0 includes Kotlin support by default
2. **Gradle 9.1.0+ required**: Update `gradle-wrapper.properties` to use Gradle 9.1.0 or higher
3. **Compose compiler plugin**: Use `org.jetbrains.kotlin.plugin.compose` instead of the old `composeOptions { kotlinCompilerExtensionVersion }` block (this change happened in Kotlin 2.0, but is now the only supported path)
4. **SDK 36**: Android 16 "Baklava" — check for new runtime permission changes
5. **KSP version alignment**: The first part of your KSP version must match your Kotlin version exactly (e.g., `ksp = "2.3.10-1.0.30"` for `kotlin = "2.3.10"`)
